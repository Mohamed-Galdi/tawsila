...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('about_pages', function (Blueprint $table) {
            ...
            $table->string('1st_FAQs')->max(255)->default('ما هي منصة "توصيلة"؟');
            $table->string('1st_answer')->max(255)->default('"توصيلة" هي منصة تطبيقات تقنية تهدف إلى تسهيل عملية نقل الطلاب إلى ومن المدرسة بوسائل النقل المدرسية بشكل آمن ومنظم.');
            $table->string('2nd_FAQs')->max(255)->default('هل يتم تخزين بياناتي الشخصية بشكل آمن على منصة "توصيلة"؟');
            $table->string('2nd_answer')->max(255)->default('نعم، نحرص على سرية وأمان بيانات مستخدمي منصة "توصيلة" وفقًا لأعلى معايير الحماية والخصوصية');
            $table->string('3rd_FAQs')->max(255)->default('كيف يمكنني الحصول على دعم فني في حالة وجود مشكلة تقنية؟');
            $table->string('3rd_answer')->max(255)->default('يمكنك الاتصال بفريق دعم العملاء لدينا عبر البريد الإلكتروني أو الهاتف المدرج في تطبيق "توصيلة" للحصول على المساعدة في حالة وجود أي مشكلة تقنية.');

            $table->string('4th_FAQs')->max(255)->default('كيف يمكنني الوصول إلى معلومات عن الرحلات والطلاب التي يجب عليّ نقلهم؟');
            $table->string('4th_answer')->max(255)->default(' بمجرد تسجيلك كسائق في توصيلة، ستحصل على حساب يمكنك استخدامه للوصول إلى معلومات عن الرحلات المقررة وتفاصيل الطلاب الذين يجب عليك نقلهم.');

            $table->string('5th_FAQs')->max(255)->default(' ما هي الإجراءات في حالة حدوث طارئ أثناء الرحلة؟');
            $table->string('5th_answer')->max(255)->default('في حالة حدوث طارئ أثناء الرحلة، يجب عليك الاتصال بفريق الدعم الفني لتوصيلة فوراً، حيث سيتم توجيهك بشكل فوري حول الإجراءات اللازمة للتعامل مع الموقف.');
            $table->string('6th_FAQs')->max(255)->default('هل توفر توصيلة دورات تدريبية للسائقين؟');
            $table->string('6th_answer')->max(255)->default('نعم، تقدم توصيلة دورات تدريبية متخصصة للسائقين تركز على السلامة والتواصل مع الطلاب ومهارات القيادة الفعالة.');
            $table->timestamps();
        });
    }
