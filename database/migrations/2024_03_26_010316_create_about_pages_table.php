...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('about_pages', function (Blueprint $table) {
            ...
            $table->text('4th_FAQs')->max(255)->default('كيف يمكنني الوصول إلى معلومات عن الرحلات والطلاب التي يجب عليّ نقلهم؟');
            $table->text('4th_answer')->max(255)->default(' بمجرد تسجيلك كسائق في توصيلة، ستحصل على حساب يمكنك استخدامه للوصول إلى معلومات عن الرحلات المقررة وتفاصيل الطلاب الذين يجب عليك نقلهم.');

            $table->text('5th_FAQs')->max(255)->default(' ما هي الإجراءات في حالة حدوث طارئ أثناء الرحلة؟');
            $table->text('5th_answer')->max(255)->default('في حالة حدوث طارئ أثناء الرحلة، يجب عليك الاتصال بفريق الدعم الفني لتوصيلة فوراً، حيث سيتم توجيهك بشكل فوري حول الإجراءات اللازمة للتعامل مع الموقف.');
            $table->text('6th_FAQs')->max(255)->default('هل توفر توصيلة دورات تدريبية للسائقين؟');
            $table->text('6th_answer')->max(255)->default('نعم، تقدم توصيلة دورات تدريبية متخصصة للسائقين تركز على السلامة والتواصل مع الطلاب ومهارات القيادة الفعالة.');
            $table->timestamps();
        });
    }
